...
\newcommand*\rectangled[1]{\tikz[baseline=(char.base)]{
        \node[shape=rectangle,fill=\sectioncolour,inner sep=2.4pt,rounded
        corners=4pt,thick]
...
\begin{cvlist}
    ...
    \cvitem{2014}{Purpose Invest}{Web}
        \blap{\llap{\makebox[2em + \widthof{AngularJS} + \widthof{Web}][l]{
            \shortstack[r]{
                \rectangled{Adobe CS} \\
                \rectangled{AngularJS}
            }
        }}}

        \input{src/purpose}
    \cvitem{2012}{PenyoPal}{Mobile}
        \blap{\llap{\makebox[2em + \widthof{MOAI SDK} + \widthof{Mobile}][l]{
            \shortstack[r]{
                \rectangled{Lua} \\
                \rectangled{MOAI SDK}
            }
